...
class MemberServiceImpl extends MembersService
{
  ...
  override def getMemberByID(id: String): List[MembersRepository#TableElementType] = {
    Database.forURL("jdbc:mysql://localhost:3306/mysql", driver = "com.mysql.jdbc.Driver", user = "root", password = "admin").withSession
    {
      implicit session =>
  ...
  override def getConversationHistory(idMember: String, idFac : String): List[MembersRepository#TableElementType] =
  {
    Database.forURL("jdbc:mysql://localhost:3306/mysql", driver = "com.mysql.jdbc.Driver", user = "root", password = "admin").withSession
    {
      implicit session =>
  ...
  override def getLastServedMember(facilitatorID : String): List[MembersRepository#TableElementType] =
  {
